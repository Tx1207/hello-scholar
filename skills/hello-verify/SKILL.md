...
声称完成之前，必须有验证证据。
运行态证据统一写入 `hello-scholar/state/`；交付证据统一写入 `hello-scholar/evidence/`；方案包统一写入 `hello-scholar/plans/`。
...
## 验证命令来源
- 当前活跃 `contract.json` 与 `tasks.md`
- package.json 中的 lint/test/typecheck/build 脚本
- pyproject.toml 中的 ruff/mypy/pytest/build 配置
...
3. 不适用的项标记 [-] 并说明原因
4. 有未通过项 → 修复 → 重新运行验证循环
5. 若当前存在方案包并准备最终收尾，优先调用 `scripts/closeout-state.mjs write` 写 `hello-scholar/state/closeout-state.json`，记录 `requirementsCoverage` 与 `deliveryChecklist` 两项结论；两项都必须包含 `status`（`PASS` / `BLOCKED`）和 `summary`
6. 若当前方案包要求 `review-first`，必须先确认 `hello-scholar/state/review-state.json` 已通过 `scripts/review-state.mjs write` 写成最新结构化证据；不要把审查自然语言消息直接当成交付证据
7. 若 `contract.json` 中 `ui.visualValidation.required=true`，必须确认 `hello-scholar/state/visual-state.json` 已通过 `scripts/visual-state.mjs write` 写成最新结构化证据；若没有视觉验收证据，不得把本轮视为 UI 可交付
...
6. 若 `contract.json` 中 `advisor.required=true` 或 `ui.styleAdvisor.required=true`，额外确认 `hello-scholar/state/advisor-state.json` 已存在且结论为 clean；若没有 advisor 证据，不得把本轮视为可交付
7. 若 `contract.json` 中 `ui.visualValidation.required=true`，额外确认 `hello-scholar/state/visual-state.json` 已存在、覆盖要求的关键 screens / states，且结论为 `PASS`；若没有视觉验收证据，不得把本轮视为 UI 可交付
8. 发现遗漏 → 补充实现 → 重新验证
