...
子代理编排必须遵循以下规范。
项目级事实源固定为 `hello-scholar/`：状态文件使用 `hello-scholar/state/STATE.md`，方案包使用 `hello-scholar/plans/`，结构化运行态使用 `hello-scholar/state/*.json`。

## 编码前
先确定任务是否适合子代理（独立性高、边界清晰、可验证）。

## 派遣规范
- 每个子代理获得：tasks.md 中的对应任务 + 方案包中的相关约束（~plan: requirements.md + plan.md；~prd: prd/ 中的相关维度文件 + decisions.md）+ 验证命令；涉及 UI 时，再附 `hello-scholar/DESIGN.md`（若存在）或其中相关片段
...
## 协调规范
- 使用子代理时，主代理作为控制器跟踪进度
- 主代理只有在本轮最终收尾时才可报告完成态。
- 团队协作中的进度与状态汇报都属于中间输出，不得伪装成完成态。
