...
使用它返回的分类、确认门、Bundle 路径和当前身份。不要复制其 ID 分配、Revision、继任关系或 Index 逻辑。若返回 `Need Human Classification`，停在该决定；若选定分类需要确认，写入前先取得确认。
...
**完成条件：** 只有选定的 `spec.md` 事务和 CLI 生成的 Index 反映已批准的设计决定。
