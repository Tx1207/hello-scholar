---
name: helloagents
description: hello-scholar 的命令兼容层与质量总则；为 commands/* 和 hello-* 质量技能提供统一约束。
---

# hello-scholar 命令兼容层

主代理触发或读取任意 skill 时，只有在该条消息是本轮最终收尾消息时，才按当前已加载 hello-scholar 规则输出最终完成态；任何 skill 若在本轮明确要求输出停顿、确认或总结，对应消息也必须同时满足相同条件。
子代理只豁免路由与收尾要求，直接执行任务；安全、质量、验证和失败处理规则仍持续生效。所有流式内容、进度或状态汇报、中间文本，以及任何仍将继续执行的文本，都不得伪装成完成态。
...
项目级事实源固定为 `hello-scholar/`。运行态文件统一写入 `hello-scholar/state/`；方案包写入 `hello-scholar/plans/`；证据写入 `hello-scholar/evidence/`；研究真相写入 `hello-scholar/research/`。`.hello-scholar/` 只承担安装态与激活态，不承载长期项目资产。
...
- 所有 UI 任务先受当前 bootstrap 的 UI 质量基线约束；已激活项目或显式 UI 工作流中的设计约束优先级固定为：当前 `plan.md` / PRD UI 决策 → `hello-scholar/DESIGN.md`（若存在） → `hello-ui` 深层规则
...
未经过完整检查就报告完成 = 违反 hello-scholar 核心规则。
...
路径定义：`{HELLO_SCHOLAR_RUNTIME_ROOT}` = 本轮已确定的 hello-scholar 读取根目录
先确定当前技能根目录：
- 优先使用当前上下文中已注入的“本轮 hello-scholar 读取根目录”
- 若当前上下文未注入，则优先使用当前项目或全局安装态中的 `hello-scholar/skills/`
- 已激活项目或全局模式下，技能是否需要使用由当前已加载 AGENTS 规则决定；不要因此额外探测其他无关路径

### hello-* 技能
读取 `{HELLO_SCHOLAR_RUNTIME_ROOT}/skills/{技能名}/SKILL.md`

### ~command 命令技能
若当前上下文已解析出具体命令技能文件路径，直接使用它；否则读取 `{HELLO_SCHOLAR_RUNTIME_ROOT}/skills/commands/{name}/SKILL.md`
