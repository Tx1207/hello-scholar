...
在 RED 之前，为本切片写明要测试的公共接口或可观察行为——即测试边界（seam）。复用用户请求或 Approved Task 已明确命名的测试边界；否则只问一个有界问题：“这个切片应验证哪个公共接口或可观察行为？”不要针对未经确认的内部协作者或私有方法编写证据。

**完成标准：** 当前切片有一个已确认的测试边界和一个可观察行为。
