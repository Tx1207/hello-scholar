...
function nextPreferenceCandidateId(paths, now = new Date()) {
  const stamp = now.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}Z$/, '').replace('T', '-')
  let suffix = 1
  let id = `PREF-${stamp}-preference`
  while (pathExists(join(paths.projectCandidatesRoot, id))) id = `PREF-${stamp}-preference-${++suffix}`
  return id
}
