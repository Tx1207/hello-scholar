...
export function reviewSkillEvolution(cwd, args) {
  ...
  const candidate = createCandidate(readCandidates(paths), {
    ...
    review: {
      ...
      activeProfiles: reviewInput.selection.activeProfiles || [reviewInput.selection.activeProfile || 'ml-development'],
...
function loadReviewInput(cwd, args) {
  ...
  const route = String(contract?.route || change.meta.route || '~build').trim() || '~build'
  ...
  const policyEnabled = contract?.skillEvolution?.enabled === true || hasSkillEvolutionCapability(selection)
  const changeVerificationCount = (change.verification || []).length
  const evidenceSatisfied = !requiresEvidence || evidence.entries.length >= minEvidenceCount || changeVerificationCount >= minEvidenceCount
...
function analyzeCandidate(input, args) {
  ...
  if (!input.policyEnabled) reasons.push('skill evolution capability 未激活，且 contract 未显式启用 skillEvolution')
...
function renderReviewMarkdown(candidate, input, decision) {
  ...
  const values = {
    ...
    profiles: (input.selection.activeProfiles || [input.selection.activeProfile || 'ml-development']).length > 0 ? (input.selection.activeProfiles || [input.selection.activeProfile || 'ml-development']).map((value) => `\`${value}\``).join(', ') : '`(none)`',
