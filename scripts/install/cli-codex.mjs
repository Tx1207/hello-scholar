...
export function readCodexStatus(runtime, installState, cwd = process.cwd()) {
  ...
  if (normalized.scope === 'global') {
    ...
    return {
      ...
      activeProfiles: hostState?.activeProfiles || globalModules?.activeProfiles || [hostState?.activeProfile || globalModules?.activeProfile || 'ml-development'],
      selectedSkills: hostState?.selectedSkills || globalModules?.skills || [],
      selectedAgents: hostState?.selectedAgents || globalModules?.agents || [],
  ...
  return {
    ...
    activeProfiles: hostState?.activeProfiles || projectModules?.activeProfiles || [hostState?.activeProfile || projectModules?.activeProfile || 'ml-development'],
    selectedSkills: hostState?.selectedSkills || projectModules?.skills || [],
    selectedAgents: hostState?.selectedAgents || projectModules?.agents || [],
    managedSkills: hostState?.managedSkills || [],
    managedAgents: hostState?.managedAgents || [],
    skippedSkills: hostState?.skippedSkills || [],
    skippedAgents: hostState?.skippedAgents || [],
...
function installCodexStandby(runtime, selection, previous, cwd = process.cwd(), catalog = null) {
  ...
  const standbyState = {
    ...
    activeProfiles: selection.activeProfiles || [selection.activeProfile || 'ml-development'],
