...
ScholarAGENTS 内置了 Obsidian 研究知识库工作流，不需要 MCP，也不需要 API key。
...
ScholarAGENTS 内置了官方 Obsidian 相关 skills 与项目级封装。
...
当 ScholarAGENTS 运行在一个包含 `scholaragents/project-memory/registry.yaml` 的仓库里时，应默认把这个仓库视为已经绑定到 Obsidian 项目知识库，并在任务过程中自动维护它。
...
当仓库已经通过 `scholaragents/project-memory/registry.yaml` 绑定后，ScholarAGENTS 默认应采用保守的自动维护策略：

- 当本轮对话改变研究状态时，至少检查 `Daily/YYYY-MM-DD.md`，
- 只有顶层项目状态真正变化时才更新 `00-Hub.md`，
- 只要项目状态变化就更新 `scholaragents/project-memory/<project_id>.md`，
...
ScholarAGENTS 可选利用官方 Obsidian CLI 与 URI：
...
| 项目反复被重新导入 | 检查 `scholaragents/project-memory/registry.yaml` 是否存在且 repo_root 是否正确 |
...
如果你在 WSL 里运行 ScholarAGENTS，但想用 **Windows 原生 Obsidian** 获得更稳定的窗口行为，推荐使用下面的双份方案：
