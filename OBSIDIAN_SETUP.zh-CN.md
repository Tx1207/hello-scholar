...
当 ScholarAGENTS 运行在一个包含 `.codex/project-memory/registry.yaml` 的仓库里时，应默认把这个仓库视为已经绑定到 Obsidian 项目知识库，并在任务过程中自动维护它。
...
当仓库已经通过 `.codex/project-memory/registry.yaml` 绑定后，ScholarAGENTS 默认应采用保守的自动维护策略：
