...
import { mergeOverlaySkill, previewRepoMerge } from '../scripts/evolution/skill-evolution-merge.mjs'
...
test('mergeOverlaySkill copies overlay skill into repo source and updates candidate state', () => {
  ...
  try {
    ...
    assert.equal(candidate.state.current, 'merged_repo')
    assert.equal(candidate.state.transitions.at(-1).name, 'merge_repo')
...
test('merge preview explains repo merge path without modifying repo', () => {
  const fixture = createFixture()
  const previousHostHome = process.env.HELLO_SCHOLAR_HOST_HOME
  const previousCodexHome = process.env.CODEX_HOME
  try {
    ...
    const result = previewRepoMerge(fixture.projectDir, parseArgv(['--candidate-id', 'skill-evo-20260416-001']), {
      pkgRoot,
      repoRoot: fixture.repoRoot,
    })
    ...
    assert.equal(result.workflow.current, 'applied_overlay')
    assert(result.notes.some((note) => note.includes('Exact next command')))
    assert.equal(existsSync(join(fixture.repoRoot, 'skills', 'overlay-skill', 'SKILL.md')), false)
  } finally {
    if (previousHostHome === undefined) {
      delete process.env.HELLO_SCHOLAR_HOST_HOME
    } else {
