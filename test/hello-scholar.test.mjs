...
test('activation prompt injects applied effective preferences but ignores pending candidates', () => {
  ...
  try {
    ...
    assert.equal((prompt.match(/^## 用户偏好$/gm) || []).length, 1)
    assert(!prompt.includes('## 当前有效用户偏好'))
    assert(prompt.includes('当前来源层：built-in、global、project'))
    assert(prompt.includes('- project-specific reviewer standard。'))
    assert(prompt.includes('- Python 包管理优先 `uv`'))
...
test('standby install writes text output, project prompt, and cleanup removes project state', () => {
  ...
  try {
    ...
    assert.equal((projectAgentsText.match(/^## 用户偏好$/gm) || []).length, 1)
    assert(!projectAgentsText.includes('## 当前有效用户偏好'))
    assert(projectAgentsText.includes('- 顶会：NeurIPS'))
    assert(projectAgentsText.includes('【hello-scholar】'))
...
test('prompt refresh updates managed block after project preference changes', () => {
  ...
  try {
    ...
    assert(projectAgentsText.includes('- refreshed reviewer standard。'))
  } finally {
    destroyFixture(fixture)
  }
})
