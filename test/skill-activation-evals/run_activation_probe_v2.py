#!/usr/bin/env python3
"""Run the approved successor Skill-catalog activation probe and retain its evidence."""
...
def require_approved_inputs(scenario_dir: Path) -> tuple[dict[str, Any], str]:
    ...
    expected = {
        ...
        "runnerSha256": sha256_file(SCRIPT_ROOT / "run_activation_probe_v2.py"),
...
def write_probe_settings(
    ...
    if protocol["scenarioId"] != "record-auto-formal-v2":
...
def main() -> int:
    ...
    parser.add_argument("scenario", choices=("record-auto-formal-v2",))
