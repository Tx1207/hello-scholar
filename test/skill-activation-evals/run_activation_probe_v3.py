#!/usr/bin/env python3
"""Run the scope-verified Skill-catalog activation probe and retain its evidence."""
...
def require_approved_inputs(scenario_dir: Path) -> tuple[dict[str, Any], str]:
    ...
    expected = {
        ...
        "runnerSha256": sha256_file(SCRIPT_ROOT / "run_activation_probe_v3.py"),
...
def write_probe_settings(
    ...
    if protocol["scenarioId"] != "record-auto-formal-v3":
        raise ValueError(f"unsupported successor Scenario: {protocol['scenarioId']}")
    ...
    allow.extend(
        [
            ...
            f"Bash({cli} docs check)",
            f"Bash({cli} docs sync)",
        ]
    )
    path.write_text(
        json.dumps(
...
def successful_matching_commands(
    events: list[dict[str, Any]], pattern: str
) -> list[str]:
    """Return successful Bash commands matching one Protocol command pattern."""

    return [
        command
        for tool_name, tool_input in successful_tool_uses(events)
        if tool_name == "Bash"
        and isinstance((command := tool_input.get("command")), str)
        and command_pattern_matches(pattern, command)
    ]


def successful_docs_sync_commands(events: list[dict[str, Any]]) -> list[str]:
    """Return successful exact absolute hello-scholar docs sync calls."""

    cli = re.escape(str(REPO_ROOT / "bin" / "hello-scholar.js"))
    pattern = re.compile(rf"^(?:node )?{cli} docs sync$")
    return [
        command
        for tool_name, tool_input in successful_tool_uses(events)
        if tool_name == "Bash"
        and isinstance((command := tool_input.get("command")), str)
        and pattern.fullmatch(command)
    ]


def changed_paths(workspace: Path) -> list[str]:
    """Return exact changed paths from NUL-delimited porcelain output."""
...
def scope_errors(protocol: dict[str, Any], workspace: Path) -> list[str]:
    """Reject every changed path outside the Protocol's explicit allow prefixes."""

    allowed = tuple(protocol["paths"]["allow"])
    denied = tuple(protocol["paths"]["deny"])
    errors: list[str] = []
    for path in changed_paths(workspace):
        if any(path == prefix.rstrip("/") or path.startswith(prefix) for prefix in denied):
            errors.append(f"denied path changed: {path}")
        elif not any(path == prefix.rstrip("/") or path.startswith(prefix) for prefix in allowed):
            errors.append(f"unexpected path changed: {path}")
    return errors


def verify_workspace(
    protocol: dict[str, Any], workspace: Path, events: list[dict[str, Any]]
) -> subprocess.CompletedProcess[str]:
    """Verify artifacts, launch count, docs ownership, and the final scoped delta."""

    errors: list[str] = []
    command_pattern = protocol["activationProbe"]["mustOccurBeforeCommand"]
    launches = successful_matching_commands(events, command_pattern)
    syncs = successful_docs_sync_commands(events)
    if len(launches) != 1:
        errors.append(f"expected one successful formal launch, observed {len(launches)}")
    if not syncs:
        errors.append("expected at least one successful absolute docs sync")
    records = sorted((workspace / "runs").glob("*/record.md"))
    if len(records) != 1 or not (workspace / "runs" / "INDEX.md").is_file():
        errors.append("expected one Run Record and runs/INDEX.md")
    errors.extend(scope_errors(protocol, workspace))
    ...
    if len(records) == 1:
        ...
        stdout_parts.append(artifact_check.stdout)
        if artifact_check.returncode != 0:
            errors.append(artifact_check.stderr.strip() or "formal artifact verifier failed")

    docs_check = subprocess.run(
        ["node", str(REPO_ROOT / "bin" / "hello-scholar.js"), "docs", "check"],
        cwd=workspace,
        capture_output=True,
        text=True,
    )
    stdout_parts.append(docs_check.stdout)
    if docs_check.returncode != 0:
        errors.append(docs_check.stderr.strip() or "evaluator docs check failed")
    if "index Current runs/INDEX.md" not in docs_check.stdout:
        errors.append("runs/INDEX.md is not Current")
    if re.search(r"^index (?:Missing|Stale) ", docs_check.stdout, re.MULTILINE):
        errors.append("one or more generated Indexes are Missing or Stale")
...
def save_result(
    ...
    launches = successful_matching_commands(events, command_pattern)
    syncs = successful_docs_sync_commands(events)
    scope_valid = not scope_errors(protocol, workspace)
    passed = (
    ...
    result = {
        ...
        "successfulLaunchCount": len(launches),
        "successfulDocsSyncCount": len(syncs),
...
def main() -> int:
    ...
    parser.add_argument("scenario", choices=("record-auto-formal-v3",))
    ...
    with tempfile.TemporaryDirectory(prefix=f"{args.scenario}-") as temp_name:
        ...
        verification = verify_workspace(protocol, workspace, events)
        save_result(
            scenario_dir,
            protocol,
            completed,
