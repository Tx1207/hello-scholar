# Formal Benchmark Automatic Activation Successor Evaluation
...
The observable activation signal is a successful `Skill` tool event for `record-experiment` before the benchmark launch. The fresh successor Fixture has its own one-launch sentinel. Both supported absolute hello-scholar CLI spellings are authorized; successful generated-Index output, not shell spelling, is the business boundary.
