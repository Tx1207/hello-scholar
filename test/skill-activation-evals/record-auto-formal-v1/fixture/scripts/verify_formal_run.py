...
def main() -> int:
    ...
    stderr = (run_dir / "logs" / "stderr.log").read_text(encoding="utf-8")
    ...
    expected_stdout = json.dumps(metrics, sort_keys=True) + "\n"
    if stdout != expected_stdout:
        fail("raw stdout does not match the benchmark process output")
    if stderr != "":
        fail("raw stderr does not match the benchmark process output")
