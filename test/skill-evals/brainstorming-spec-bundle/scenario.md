# Brainstorming: Evolve An Existing Ranking Pipeline Design
...
2. Ask one contract-relevant question at a time. The Eval main agent supplies future answers only after each real stop.
3. Present two or three materially different approaches with a recommendation and tradeoffs.
4. Before writing, enter the `manage-specs` identity step. The Baseline intentionally has no such Skill; do not invent or search for it elsewhere.
...
6. After the future whole-Spec approval, write exactly the reviewed revision, run the required checks, and self-review the saved file. The user's stated endpoint is design-only, so do not enter `writing-plans` afterward.

The Eval harness supplies immutable pre-change copies of `brainstorming` and `writing-plans`. Explicit Skill loading does not authorize the wrong terminal branch.
...
Run `python3 -m unittest discover -s tests` and the absolute `hello-scholar docs check`. Runtime bytes remain unchanged and generated Indexes are current.

## Interaction

The future rounds answer one clarification, select an approach, and approve one complete revised Spec for writing. None of those replies are included in the first prompt.
