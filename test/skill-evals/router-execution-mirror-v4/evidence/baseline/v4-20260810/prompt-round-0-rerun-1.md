...
`/tmp/hello-scholar-eval-router-execution-mirror-v4-baseline-rerun1-tT6621qO`

Every shell command must explicitly run in that directory with `env -C /tmp/hello-scholar-eval-router-execution-mirror-v4-baseline-rerun1-tT6621qO ...`, `git -C /tmp/hello-scholar-eval-router-execution-mirror-v4-baseline-rerun1-tT6621qO ...`, or an equivalent explicit working-directory parameter. Shell working directories do not persist between calls.

Before acting, read the project-local `AGENTS.md` and only files inside the isolated project needed for this request. Do not read the source repository, Eval Scenario or Protocol, rubric, production Skill, external Task Packet, previous Eval evidence, or any path outside the isolated project except the Skill snapshot below.
