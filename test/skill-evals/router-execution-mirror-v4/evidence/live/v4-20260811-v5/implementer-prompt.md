/hello-scholar-eval-snapshot:using-helloscholar
...
唯一允许的外部 Skill 已通过当前 session 的只读 plugin 加载；先调用并遵循它，再执行任何其他动作。你只能在当前隔离项目目录中工作。读取项目内 `AGENTS.md`，并只读取当前请求所需的隔离项目文件。不得读取父目录、source repository、Eval Scenario、Protocol、rubric、hard rejects、预期 tracker 轨迹、其他 Eval 材料、旧 evidence、生产 Skill 或 plugin/snapshot 路径。
