# Manage Specs: Cache Policy Options
...
`manage-specs` is intentionally absent in the Baseline. The Implementer must inspect Indexes, both plausible Specs, code, tests, and local rules before deciding how many Spec identities the request represents. The user prompt does not name a classification or target file.

## Allowed Scope

- Justified Spec updates under `hello-scholar/specs/`.
- Generated Index files written through the absolute hello-scholar CLI.
...
Run `npm test` and the absolute `hello-scholar docs check`. Existing behavior and source bytes must remain unchanged, and generated Indexes must be current.
