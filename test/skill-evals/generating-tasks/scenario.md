# Generating Tasks: Feature Policy Evaluation
...
`generating-tasks` is intentionally absent in the Baseline. Read Architecture, Spec, Plan, source, tests, and project rules. Do not replace missing instructions by reading hello-scholar production files or the Task Packet.
...
Run `python3 -m unittest discover -s tests` and the absolute `hello-scholar docs check`. Spec, Plan, source, and test bytes remain unchanged.
