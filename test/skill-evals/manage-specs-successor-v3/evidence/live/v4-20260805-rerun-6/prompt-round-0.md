...
For this round, stay at the requested classification stop and make zero writes. Return exactly one classification, candidate evidence, the complete canonical paths affected by the proposed successor transaction, and stop point. Preserve the complete public design identity. A later user message may arrive only after this stop; when it does, continue this same finite interaction under these same read/write boundaries rather than rejecting it as another workflow. Do not assume or reveal that later message now.
