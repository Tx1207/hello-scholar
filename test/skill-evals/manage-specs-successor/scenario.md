# Manage Specs: Successor Session Token Model
...
Round one is read-only. Inspect Indexes, both plausible Specs, code, tests, and local rules; report one evidence-backed classification and stop. The future decision is not included here. `manage-specs` is intentionally absent in the Baseline.

After the Eval main agent approves the current classification, update only the relationship-bearing Spec documents and generated Indexes that classification requires.
...
- Any write before the future classification approval.
...
Run `npm test` and the absolute `hello-scholar docs check`. Existing runtime behavior stays unchanged, supersession is reciprocal and acyclic, and generated Indexes are current.
