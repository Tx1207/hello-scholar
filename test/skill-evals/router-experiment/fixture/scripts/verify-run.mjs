...
const sentinel = JSON.parse(await readFile(path.join(runAbsolute, ".launch-sentinel"), "utf8"));
const rawText = await readFile(path.join(runAbsolute, "outputs/benchmark.json"), "utf8");
const metricsText = await readFile(path.join(runAbsolute, "results/metrics.json"), "utf8");
...
assert.equal(raw.checksum, 40000);
assert.equal(metrics.checksum, raw.checksum);
assert.equal(metrics.query_count, raw.query_count);
assert.equal(metrics.exit_code, 0);
assert.equal(sentinel.command, expectedCommand);
assert.equal(sentinel.started_at, raw.started_at);
assert.equal(sentinel.record_sha256_at_start, raw.record_sha256_at_start);
assert.match(sentinel.record_sha256_at_start, /^[0-9a-f]{64}$/);
assert.notEqual(
  createHash("sha256").update(record).digest("hex"),
  sentinel.record_sha256_at_start,
