...
| REQ-005 | Agent 内部七个命令作为任务路由，不作为 npm 子命令。 | Codex 会话输入 `~idea`、`~plan`、`~build`、`~verify`、`~analyze`、`~evolve`、`~apply-candidate`。 | 根据命令可能写 `hello-scholar/plans`、`changes`、`experiments`、`state`、`evolution`、`preferences/candidates`，或在确认后同步 skill / preference 目标文件。 | 七个 command skill 均存在；每个命令声明触发、边界和输出。 | Done | `skills/commands/*/SKILL.md` | 已补充 candidate 审核与同步命令。 |
...
| 候选应用 | `~apply-candidate` | 可能 | candidate 状态、change record；确认后同步 overlay/repo skill 或 `user-preferences.yaml` | 默认只 review/preview；必须明确 approve 后才 apply。 |
