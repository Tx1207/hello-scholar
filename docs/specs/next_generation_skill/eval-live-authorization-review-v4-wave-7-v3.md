# Execution Mirror Haiku v4 Top-level Live authorization v3

- Status: `historical-stale-after-skill-repair`
- Batch ID: `haiku-v4-wave-7-execution-mirror-live-authorization-v3`
- Batch SHA-256: `sha256:b3d1e01014533d2db7f7a8c1344f27d3dcf4621fdecf83bc5f8410753ba61093`
- Manifest: [`eval-live-authorization-batch-v4-wave-7-v3.json`](./eval-live-authorization-batch-v4-wave-7-v3.json)
- Scope: run the unchanged execution-mirror case in fresh top-level Claude Code CLI sessions rather than Agent subagents.
...
- Authorization basis: Scenario, Protocol, Fixture, shared rubric, genuine Red Baseline, and current Skill snapshot remain bound; the transport change is explicit and immutable.

This authorization permits one fresh top-level Haiku Implementer session followed strictly serially by a different fresh top-level Haiku Reviewer session. It does not accept Skill output or authorize unrelated changes. The resulting Scorecard starts with `userDecision: pending`.
...
- Live approval ID: [`live-router-execution-mirror-v4-haiku-v4-v3`](../../../test/skill-evals/router-execution-mirror-v4/live-approval.json)
