...
ScholarAGENTSが`.codex/project-memory/registry.yaml`を含むリポジトリ内で実行されている場合、そのリポジトリをObsidianプロジェクトナレッジベースにバインド済みとして扱い、デフォルトで更新を行います。
...
リポジトリが`.codex/project-memory/registry.yaml`を通じて既にバインドされている場合、ScholarAGENTSは自動メンテナンスを控えめに行います:
